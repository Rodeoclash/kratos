...
var credentialsTypes map[identity.CredentialsType]*identity.CredentialsTypeTable

func init() {
	credentialsTypes = make(map[identity.CredentialsType]*identity.CredentialsTypeTable)
}
func (p *IdentityPersister) findIdentityCredentialsType(ctx context.Context, ct identity.CredentialsType) (_ *identity.CredentialsTypeTable, err error) {
	...
	if v, ok := credentialsTypes[ct]; ok && v != nil {
		return v, nil
	}
...
func (p *IdentityPersister) createIdentityCredentials(ctx context.Context, identities ...*identity.Identity) (err error) {
	...
	// create identifiers in bulk
	if err := c.Create(credentials); err != nil {
		return err
	}
	...
	if err := c.Create(identifiers); err != nil {
		return err
	}
...
func (p *IdentityPersister) CreateIdentities(ctx context.Context, identities identity.Identities) (err error) {
	...
	return p.Transaction(ctx, func(ctx context.Context, tx *pop.Connection) error {
		ptrToIdentities := make([]*identity.Identity, len(identities))
		if err := tx.Create(identities); err != nil {
			return sqlcon.HandleError(err)
		}

		for i, ident := range identities {
			p.normalizeAllAddressess(ctx, &ident)
			ptrToIdentities[i] = &ident
		}

		eg, ctx := errgroup.WithContext(ctx)

		eg.Go(func() error {
			return p.createVerifiableAddresses(ctx, identities.VerifiableAddresses())
		})
		eg.Go(func() error {
			return p.createRecoveryAddresses(ctx, identities.RecoveryAddresses())
		})
		eg.Go(func() error {
			return p.createIdentityCredentials(ctx, ptrToIdentities...)
		})
...
func (p *IdentityPersister) UpdateIdentity(ctx context.Context, i *identity.Identity) (err error) {
	...
	return sqlcon.HandleError(p.Transaction(ctx, func(ctx context.Context, tx *pop.Connection) error {
		...
		return sqlcon.HandleError(p.createIdentityCredentials(ctx, i))
	}))
}
