...
func TestStrategy(t *testing.T) {
	...
	t.Run("case=api should fail because id_token was not provided", func(t *testing.T) {
		...
		body := expectValidationError(t, true, false, false, values)
	...
	t.Run("case=upstream parameters should be passed on to provider", func(t *testing.T) {
		...
		t.Run("case=should pass when registering", func(t *testing.T) {
			f := newRegistrationFlowBrowser(t, returnTS.URL, time.Minute)
		...
		t.Run("case=should pass when logging in", func(t *testing.T) {
			f := newLoginFlowBrowser(t, returnTS.URL, time.Minute)
